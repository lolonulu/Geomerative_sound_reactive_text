...
class SoundManager {
  ...
  void update() {
    ...
    if ( indexPhraseSet==-1) { 
      ...
      // lecture son terminée
      indexPhrase++;
        
      if (indexPhrase >= sounds[indexPhraseSet].length) {
        // là on passe à la strophe suivante
        if (millis() < millis()-startTime+PAUSE_DURATION ) {
        ...
        // replacer les lignes du fond à leur position origine
        ...
        if (indexPhraseSet >= sounds.length) {
          indexPhraseSet=0;
          //on recommence la lecture dès le début
        }
        // pause
